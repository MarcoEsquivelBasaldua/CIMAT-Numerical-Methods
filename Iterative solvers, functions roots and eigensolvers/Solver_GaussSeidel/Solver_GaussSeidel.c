/*
*   Code name: Solver_GaussSeidel.c
...
*      calculates the values of x from the equation Ax = b, by Gauss-Seidel's method that is an iterative method
*      used to find the values of the unknowns
*      Gauss-Seidel's method is very similar to Jacobi's method, but differing in that the solutions to the unknowns use calculations made in the very same
*      iteration, not all from the previous one like Jacobi's method does. This enables to accelerate convergence to solutions
*/
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
void Solver_GaussSeidel(double**, int, int, double**,int, int, double, int);     // Function to find the values of the unknowns by Gauss-Seidel's method
...
int main( int argc , char* argv[] ){
    ...
    Solver_GaussSeidel(A, n, m, b, k, l, toler, max_iter);
...
void Solver_GaussSeidel(double** A, int n, int m, double** b, int k, int l, double toler, int max_iter){
    /*
*   Inputs:
...
*/
    ...
    if(flag == 1){
        double xO[n];//double* xO = malloc(sizeof(double) * n);    // Solutions to unknowns in the previous iteration are stored here
        double xN[n];//double* xN = malloc(sizeof(double) * n);    // Solutions to unknowns in the current iteration are stored here

        // xO and xN initialization, all entries are set to 1
        // xN has to be initialized since it will be used to find solutions since the begining, this is different to Jacobi's method
        for(int i=0; i<n; i++){
            xO[i] = 1.0;
            xN[i] = 1.0;
        }
        ...
        while(count_iter <= max_iter && aux > toler){
            ...
            // New values for the unknowns are calculated given the values gotten in the previous iteration  according to Gauss-Seidel's method
            for(int i=0; i<n; i++){
                double sum =0;

                for(int j=0; j<n; j++) if(i != j) sum += A[i][j]*xN[j];

                xN_i = (b[i][0]-sum)/A[i][i];
                xO_i = xO[i];

                xN[i] = xN_i;

                if(xN_i != 0) aux += (xN_i-xO_i)*(xN_i-xO_i)/(xN_i*xN_i);    // if condition in this line avoids division by cero
            }
            ...
            if(aux > toler){
                if(count_iter < max_iter)  memcpy(xO,xN,n*sizeof(double));// for(int i=0; i<n; i++) xO[i] = xN[i];                     // xO is prepeared for next iteration taking xn values
