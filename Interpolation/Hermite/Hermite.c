...
*       According to user selection, points in "Enter points.txt" are interpolated in a given funtion by Hermite method.
...
void Hermite(double*,int,int);     // Interpolation by Hermite Method
...
int main(int argc, char* argv[]){
    ...
    // Interpolation by Hermite Method
    // Time excecution starts
    clock_t t_ini, t_fin;
    double secs;
    t_ini = clock();

    Hermite(x_i,n,f_selc);
    printf("\n");
...
void Hermite(double* x_i,int n, int f_selec){
/*
*   Inputs:
...
*/
    ...
    for(int j=0; j<=m; j++){
        ...
        for(int i=0; i<n; i++){
            ...
            for(int k=0; k<n; k++){
                if(k != i)
                    li *= (x[j] - x_i[k])/(x_i[i] - x_i[k]);
            }

            // Lagrange coefficient first derivative
            double li_prime = 0;
            for(int p=0; p<n; p++){
                if(p!=i){
                    double l_aux = 1;
                    //if(p != i){
                        //for(int m=0; m<n; m++){
                            //if(m !=p && m!=i) l_aux *= (x_i[i] - x_i[m])/(x_i[i]-x_i[m]);
                        //}
                    //}
                    li_prime += l_aux/(x_i[i] - x_i[p]);
                }
            }

            aux_y += (-2*li_prime*x[j] + 1 + 2*x_i[i]*li_prime)*li*li*f_x(f_selec,x_i[i]) + (x[j] - x_i[i])*li*li*f_x_prime(f_selec,x_i[i]);
        }
        P[j] = aux_y;
    }
