/*
*   Code name: GaussLegendre_n=1.c
*   Author: Marco Antonio Esquivel Basaldua
*   Date: 05/11/19
*
*   Code description:
*       User selects a function present in the start menu to get its integral value
*       by Gauss-Legendre method for the given interval using n=1.
...
void Integral_n1(int,int);                     // Function to calculate integral
...
int main(void){
    ...
    while(f_selc < 1 && f_selc > 9){
        printf("ERROR: option %d is not avaiable, try again.\n",f_selc);
        scanf("%d",&f_selc);
    }

    int n;
    printf("\nEnter the number of points to approximate the integral (n = 0 to 4):\n");

    scanf("%d",&n);
    while(n < 0 && n > 4){
        printf("ERROR: option %d is not valid, try again.\n",n);
        scanf("%d",&n);
    }
...
void Integral_n1(int f_selec, int n){
/*
*   Inputs:
*       - int f_selec: this value corresponds to the math function to work with according to the menu
*
...
*/
    ...
    switch (n){
        ...
        break;
    case 2:
        x_i[0]=-sqrt(1.0/3.0); x_i[1]=-x_i[0];
        w_i[0] = 1; w_i[1] = 1;
        break;
    case 3:
        x_i[0]=-sqrt(3.0/5.0); x_i[1]=0.0; x_i[2]=sqrt(3.0/5.0);
        w_i[0]=5.0/9.0; w_i[1]=8.0/9.0; w_i[2]=5.0/9.0;
        break;
    case 4:
        x_i[0]=-sqrt((3.0+2.0*sqrt(6.0/5.0))/7.0); x_i[1]=-sqrt((3.0-2.0*sqrt(6.0/5.0))/7.0); x_i[2]=-x_i[1]; x_i[3]=-x_i[0];
        w_i[0]=(18.0-sqrt(30.0))/36.0; w_i[1]=(18.0+sqrt(30.0))/36.0; w_i[2]=(18.0+sqrt(30.0))/36.0; w_i[3]=(18.0-sqrt(30.0))/36.0;
        break;
    case 5:
        x_i[0]=0; x_i[1]= (1.0/3.0)*sqrt(5.0-2.0*sqrt(10.0/7.0)); x_i[2]=-x_i[1]; x_i[3]=(1.0/3.0)*sqrt(5.0+2.0*sqrt(10.0/7.0)); x_i[4]=-x_i[3];
        w_i[0]=128.0/225.0; w_i[1]=(322.0+13.0*sqrt(70))/900.0; w_i[2]=w_i[1]; w_i[3]=(322.0-13.0*sqrt(70))/900.0; w_i[4]=w_i[3];
    default:
        break;
    }
