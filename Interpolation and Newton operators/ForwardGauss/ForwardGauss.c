...
void ForwarGauss(double,double,int,int); // Interpolation by Gauss forward central differences method
...
void ForwarGauss(double x_0, double x_n, int n, int f_selec){
/*
*   Inputs:
...
*/
    ...
    // Gauss Forward Method in action
...
///////
    ...
    for(int j=0; j<=m; j++){
        double s;
        if(n%2 != 0) s = (x[j] - x_i[n/2])/h;
        else s = (x[j] - x_i[n/2-1])/h;
        ...
        for(int i=1; i<n; i++){
            ...
            if(n%2 != 0){
                if(i%2 != 0){
                    aux_1 = FD[n/2 - pos][i] /factorial(i);
                    aux_2 = aux_2_old * (s + pos++);
                }
                else{
                    aux_1 = FD[n/2 - pos][i] /factorial(i);
                    aux_2 = aux_2_old * (s - pos);
                ...
                if(i%2 != 0){
                    aux_1 = FD[n/2 - pos-1][i] /factorial(i);
                    aux_2 = aux_2_old * (s + pos++);
                }
                else{
                    aux_1 = FD[n/2 - pos-1][i] /factorial(i);
        ...
        if(n%2 != 0) P[j] = aux + FD[n/2][0];
        else P[j] = aux + FD[n/2-1][0];
    }
