...
void Stirling(double,double,int,int); // Interpolation by Stirling Method
...
void Stirling(double x_0, double x_n, int n, int f_selec){
/*
*   Inputs:
...
///////
    ...
    for(int j=0; j<=m; j++){
        ...
        double aux_2;
        double aux_2_old = 1;
        int pos = 0, impar = 0, par = 0;
        for(int i=1; i<n; i++){
            aux_1 = 1;

            if(i%2 != 0){
                impar++;
                for(int k=1; k<impar; k++) aux_1 *= (s*s -k*k);
                aux_1 *= s;
                aux_1 = aux_1/factorial(i);
                aux_2 = (FD[n/2-pos][i] + FD[n/2-1-pos][i])/2;
                pos++;
            }
            else{
                par++;
                for(int k=1; k<par; k++) aux_1 *= (s*s -k*k);
                aux_1 *= s*s;
                aux_1 = aux_1/factorial(i);
                aux_2 = FD[n/2-pos][i];
            }
