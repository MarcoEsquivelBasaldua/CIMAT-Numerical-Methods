/*
*   Code name: BackwardNewton.c
*   Author: Marco Antonio Esquivel Basaldua
*   Date: 26/10/19
*
*   Code description:
*       This code captures the value n that correspondes to the total of interpolation points (n+1),
...
*       Gregory-Newton backward method.
...
void BackwardNewton(double,double,int,int);   // Interpolation by Gregory-Newton Backward method
...
int main(int argc, char* argv[]){
    ...
    BackwardNewton(x_0,x_n,n,f_selc);
    printf("\n");
...
void BackwardNewton(double x_0, double x_n, int n, int f_selec){
/*
*   Inputs:
...
*/
    ...
    // Gregory-Newton Backward Method in action

    // Create matrix to store forward differences
    double **BD = (double **)malloc(n * sizeof(*BD));
    for(int j=0; j<n; j++) BD[j] = (double *) malloc((n-j) * sizeof(*BD[j]));

    for(int j=0; j<n; j++){
        for(int i=0; i<(n-j); i++){
            if(j == 0) BD[i][j] = f_x(f_selec,x_i[n-i-1]);
            else BD[i][j] = BD[i][j-1] - BD[i+1][j-1];
        }
    }

    /////
    printf("\nBackward Differences generated\n");
    for(int i=0; i<n; i++){
        for(int j=0; j<(n-i); j++){
            printf("%lf ",BD[i][j]);
        }
...
///////
    ...
    for(int j=0; j<=m; j++){
        ...
        double aux_2_old = 1;
        for(int i=1; i<n; i++){
            aux_1 = BD[0][i] /(factorial(i)*pow(h,i));

            aux_2 = aux_2_old * (x[j] - x_i[n-i]);
            aux_2_old = aux_2;
    ...
    for(int i=0;i<n;i++) free(BD[i]); // Se libera la memoria solicitada
    free(BD);
}
