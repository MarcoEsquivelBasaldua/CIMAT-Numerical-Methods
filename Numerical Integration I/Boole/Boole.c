...
*       by Boole method for the given interval.
...
double Boole(double,double,int,int); // Method used to get the integral
...
int main(void){
    ...
    // n needs a multiple of 4, if it is not it must be adjusted
    if(m%4 != 0){
        if(m%4 == 1) m += 3;
        else if(m%4 == 2) m += 2;
        else m += 1;
...
double Boole(double x_0,double h,int m,int f_selec){
/*
*   Inputs:
...
*
*   Output:
*       - double integ: approximation to the integral by Boole method
*/
    double integ = 0;

    for(int i=0; i<=m; i++){
        double x_i = x_0 +i*h;
        if(i==0 || i==m) integ += 7*f_x(f_selec,x_i);
        else if(i%2 == 1) integ += 32*f_x(f_selec,x_i);
        else if(i%4 == 0) integ += 14*f_x(f_selec,x_i);
        else integ += 12*f_x(f_selec,x_i);
...
*/
    ...
    integ = Boole(x_0,h,m,f_selec);

    // Integral by Boole rule
