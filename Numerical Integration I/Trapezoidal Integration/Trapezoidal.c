...
*       by Trapezoidal method for the given interval.
...
double Trapezoidal(double x_0,double h,int m,int f_selec){
/*
*   Inputs:
...
*
*   Output:
*       - double integ: approximation to the integral by Trapezoidal method
*/
    ...
    for(int i=0; i<=m; i++){
        ...
        if(i == 0) integ += f_x(f_selec,x_i)/2;
        else if (i == m) integ += f_x(f_selec,x_i)/2;
        else integ += f_x(f_selec,x_i);
    }
    integ *= h;
    return integ;
}
...
*/
    ...
    double integ = Trapezoidal(x_0,h,m,f_selec);
