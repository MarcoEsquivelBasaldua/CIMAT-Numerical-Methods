...
*       by Weddle method for the given interval.
...
double Weddle(double,double,int,int); // Method used to get the integral
...
int main(void){
    ...
    // n needs a multiple of 6, if it is not it must be adjusted
    if(m%6 != 0){
        if(m%6 == 1) m += 5;
        else if(m%6 == 2) m += 4;
        else if(m%6 == 3) m += 3;
...
double Weddle(double x_0,double h,int m,int f_selec){
/*
*   Inputs:
...
*
*   Output:
*       - double integ: approximation to the integral by Weddle method
*/
    double integ = 0;
    m = m/6;

    for(int i=0; i<m; i++){
        integ += ((3*h/10)*(f_x(f_selec,x_0) + f_x(f_selec,x_0+2*h) + 5*f_x(f_selec,x_0+h)+ 6*f_x(f_selec,x_0+3*h)+ f_x(f_selec,x_0+4*h)+ 5*f_x(f_selec,x_0+5*h)+f_x(f_selec,x_0+6*h)));
        x_0 = x_0+6*h;
    }
...
*/
    ...
    integ = Weddle(x_0,h,m,f_selec);

    // Integral by Weddle rule
