...
*       by Simpson 1/3 method for the given interval.
...
double Simpson1_3(double,double,int,int); // Method used to get the integral
...
int main(void){
    ...
    // m needs to be even, if it is not it must be adjusted
    if(m%2 != 0){
        m++;
...
double Simpson1_3(double x_0,double h,int m,int f_selec){
/*
*   Inputs:
...
*
*   Output:
*       - double integ: approximation to the integral by Simpson 1/3 method
*/
    ...
    for(int i=0; i<=m; i++){
        ...
        else if(i%2 == 0) integ += 2*f_x(f_selec,x_i);
        else integ += 4*f_x(f_selec,x_i);
    }
...
*/
    ...
    integ = Simpson1_3(x_0,h,m,f_selec);
